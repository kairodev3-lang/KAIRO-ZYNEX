async function command(sock, from, msg, q, createPairing) {
    ...
    if (!/^\d{8,15}$/.test(number)) return sock.sendMessage(from, { text: '*❌ ᴜsᴀɢᴇ: .ᴘᴀɪʀ 509XXXXXXXX*\n*ɪɴᴄʟᴜᴅᴇ ᴛʜᴇ ᴄᴏᴜɴᴛʀʏ ᴄᴏᴅᴇ.*' }, { quoted: msg });
    try { await createPairing(number, from); }
    catch (e) { await sock.sendMessage(from, { text: `*❌ ᴘᴀɪʀɪɴɢ ғᴀɪʟᴇᴅ:* ${e.message}` }, { quoted: msg }); }
}
