    ...
    try {
        ...
        if (!message.key.fromMe) {
            await sock.sendMessage(chatId, {
                text: '*❌ ᴛʜɪs ᴄᴏᴍᴍᴀɴᴅ ɪs ᴏɴʟʏ ᴀᴠᴀɪʟᴀʙʟᴇ ғᴏʀ ᴛʜᴇ ᴏᴡɴᴇʀ!*',
                contextInfo: {
                    forwardingScore: 1,
                    isForwarded: true,
                    forwardedNewsletterMessageInfo: {
                        newsletterJid: '120363424818286187@newsletter',
                        newsletterName: '©𝙾𝙵𝙵𝙸𝙲𝙸𝙰𝙻 𝙲𝙷𝙰𝙽𝙽𝙴𝙻',
                        serverMessageId: -1
                    }
                }
            });
        ...
        if (args.length > 0) {
            ...
            if (action === 'on' || action === 'enable') {
                ...
                await sock.sendMessage(chatId, {
                    text: '*❌ ɪɴᴠᴀʟɪᴅ ᴏᴘᴛɪᴏɴ! ᴜsᴇ: .ᴀᴜᴛᴏʀᴇᴀᴅ ᴏɴ/ᴏғғ*',
                    contextInfo: {
                        forwardingScore: 1,
                        isForwarded: true,
                        forwardedNewsletterMessageInfo: {
        ...
        await sock.sendMessage(chatId, {
            text: `*✅ ᴀᴜᴛᴏ-ʀᴇᴀᴅ ʜᴀs ʙᴇᴇɴ ${config.enabled ? 'enabled' : 'disabled'}!*`,
            contextInfo: {
                forwardingScore: 1,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
        ...
        console.error('ᴇʀʀᴏʀ ɪɴ ᴀᴜᴛᴏʀᴇᴀᴅ ᴄᴏᴍᴍᴀɴᴅ:', error);
        await sock.sendMessage(chatId, {
            text: '❌ ᴇʀʀᴏʀ ᴘʀᴏᴄᴇssɪɴɢ ᴄᴏᴍᴍᴀɴᴅ!',
            contextInfo: {
                forwardingScore: 1,
                isForwarded: true,
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363424818286187@newsletter',
                    newsletterName: '©𝙾𝙵𝙵𝙲𝙸𝙰𝙻 𝙲𝙷𝙰𝙽𝙽𝙴𝙻',
                    serverMessageId: -1
                }
            }
        });
...
function isAutoreadEnabled() {
    try {
        ...
        console.error('Error ᴄʜᴇᴄᴋɪɴɢ ᴀᴜᴛᴏʀᴇᴀᴅ sᴛᴀᴛᴜs:', error);
        return false;
    }
}
...
function isBotMentionedInMessage(message, botNumber) {
    ...
    if (textContent) {
        ...
        const botNames = [global.botname?.toLowerCase(), 'bot', '𝙳𝚁𝚄𝚉𝚉 𝚇-𝙼𝙳', '𝙳𝚁𝚄𝚉𝚉 𝚇-𝙼𝙳'];
