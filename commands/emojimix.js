...
async function emojimixCommand(sock, chatId, msg) {
    try {
        ...
        if (!args[0]) {
            await sock.sendMessage(chatId, { text: '*🎴 ᴇxᴀᴍᴘʟᴇ: .ᴇᴍᴏᴊɪᴍɪx 😎+🥰*' });
            return;
        }

        if (!text.includes('+')) {
            await sock.sendMessage(chatId, { 
                text: '*✳️ sᴇᴘᴀʀᴀᴛᴇ ᴛʜᴇ ᴇᴍᴏᴊɪ ᴡɪᴛʜ ᴀ *+* sɪɢɴ*\n\n`📌 ᴇxᴀᴍᴘʟᴇ:` \n*.ᴇᴍᴏᴊɪᴍɪx* 😎+🥰' 
            });
            return;
        }
        ...
        if (!data.results || data.results.length === 0) {
            await sock.sendMessage(chatId, { 
                text: '*❌ ᴛʜᴇsᴇ ᴇᴍᴏᴊɪs ᴄᴀɴɴᴏᴛ ʙᴇ ᴍɪxᴇᴅ! ᴛʀʏ ᴅɪғғᴇʀᴇɴᴛ ᴏɴᴇs.*' 
            });
            return;
        }
        ...
        try {
            ...
            console.error('ᴇʀʀᴏʀ ᴄʟᴇᴀɴɪɴɢ ᴜᴘ ᴛᴇᴍᴘ ғɪʟᴇs:', err);
        }

    } catch (error) {
        console.error('ᴇʀʀᴏʀ ɪɴ ᴇᴍᴏᴊɪᴍɪx ᴄᴏᴍᴍᴀɴᴅ:', error);
        await sock.sendMessage(chatId, { 
            text: '*❌ ғᴀɪʟᴇᴅ ᴛᴏ ᴍɪx ᴇᴍᴏᴊɪs! ᴍᴀᴋᴇ sᴜʀᴇ ʏᴏᴜ\'re ᴜsɪɴɢ ᴠᴀʟɪᴅ ᴇᴍᴏᴊɪs.*\n\n*ᴇxᴀᴍᴘʟᴇ: .ᴇᴍᴏᴊɪᴍɪx* 😎+🥰' 
        });
    }
}
